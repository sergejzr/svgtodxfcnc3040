...
class CutRepeater:
    ...
    def __call__(self, value):
        if self.support:
            return self.addears(value, self.support)
    ...
    def addears(self,match, support,  passes):
        ...
        if lline:
            ...
            supportsplit = self.splitinthemiddle(a, b, support["support_gap"])
...
class Svg2GcodeMerger:
    ...
    def svg2cleangc(self, newline, speed, strength, codestyle, repeat, addsuport=False):
        ...
        if addsuport:
            rptr = CutRepeater(repeat, {"gap":addsuport["support_gap"],"laseron":strength,"laseroff":addsuport["support_strength"]})
    ...
    def fromCommandline(self):
        ...
        parser.add_argument('-o', '--output', type=str, nargs='+', required=True, help='output file *.gcode')
        ...
        self.args = args
        return self.process(args.output[0], inputarray,
